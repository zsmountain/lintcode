...
class Solution:
    ...
    def numUniqueEmails(self, emails):
        ...
        uniq_emails = set()
        for email in emails:
            uniq_emails.add(self.trim_email(email))
        return len(uniq_emails)

    def trim_email(self, email):
        ...
        for ch in local_name:
            ...
            if ch == '+':
                break
            local_name_chars.append(ch)
        return ''.join(local_name_chars) + '@' + domain_name

s = Solution()
print(s.numUniqueEmails(['test.email+alex@lintcode.com', 'test.e.mail+bob.cathy@lintcode.com', 'testemail+david@lin.tcode.com']))